...
def _get_client():
    ...
    if _client is None:
        if not HF_API_TOKEN:
            raise RuntimeError("Set HF_API_TOKEN in yout .env")
...
def _product_text(product:Product):
...
def _normalized_embedding(raw_embedding):
...
def _embed_text(client:InferenceClient, text:str):

    raw = client.feature_extraction(text, model = HF_RECOMMENDER_MODEL)
    return _normalized_embedding(raw)

def _embed_products(client: InferenceClient, products: list[Product]):
...
def _fallback_recommendation(products: list[Product], top_k: int):
    fallback = [(product, 0.0) for product in sorted(products, key = lambda p: p.id, reverse=True)[:top_k]]
    return "cold_start_fallback", fallback

def _popularity_scores(db:Session):
...
def _rank_products(
    ...
    for product in products:
        semantic_score = _cosine_similarity(profile_vector, embeddings[product.id])
        boost = 0.0
...
def generate_customer_recommendations(
    ...
    interacted_ids = {int(pid)for pid in ordered_product_ids + cart_product_ids}
    if not interacted_ids:
        return _fallback_recommendation(products, top_k)
    
    products_by_id = {product.id: product for product in products}
    active_interacted_products = [products_by_id[pid] for pid in interacted_ids if pid in products_by_id]
    if not active_interacted_products:
        return _fallback_recommendation(products, top_k)
    
    candidate_products = [product for product in products if product.id not in interacted_ids]
    if not candidate_products:
        return "customer_ai_profile",[]
    
    client = _get_client()
    embeddings = _embed_products(client, active_interacted_products + candidate_products)

    profile_vectors = [embeddings[product.id]for product in active_interacted_products]
...
def generate_guest_recommendations(
    ...
    top_seed_ids = [
        product_id
        for product_id, _score in sorted(popularity_scores.items(),key = lambda item: item[1], reverse=True)[:3]
        if product_id in products_by_id
    ]

    seed_products = [products_by_id[product_id]for product_id in top_seed_ids]
    if not seed_products:
        return _fallback_recommendation(products,top_k)
    
    embeddings = _embed_products(client, products)
    seed_vectors = [embeddings[product.id] for product in seed_products]
